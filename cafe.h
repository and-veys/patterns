#include <iostream>
#include <vector>
class Meal {
public:
	virtual float getPrice() = 0;
	virtual std::string getName() = 0;
	virtual void info() = 0;
	virtual ~Meal(){};
};

class Pancake : public Meal {
private:
   std::string name;
   float price;
public:
   Pancake(const std::string &name, float price) : name(name), price(price) {}
   float getPrice() override {
       return price;
   }
   std::string getName() override{
       return name;
   }
   void info() override {
		std::cout << getName() << ", price: " << getPrice() << std::endl;
   }
};

class MorningPancake : public Pancake {
public:
	MorningPancake() : Pancake("Morning pancake", 240.0f) {}
};

class LunchPancake : public Pancake {
public:
	LunchPancake() : Pancake("Lunch pancake", 280.0f) {}
};

class MexicanPancake : public Pancake {
public:
	MexicanPancake() : Pancake("Mexican pancake", 270.0f) {}
};

class PotatoPancake : public Pancake {
public:
	PotatoPancake() : Pancake("Potato pancake", 210.0f) {}
};

class MushroomPancake : public Pancake {
public:
	MushroomPancake() : Pancake("Mushroom pancake", 220.0f) {}
};

class FalafelPancake : public Pancake {
public:
	FalafelPancake() : Pancake("Falafel pancake", 270.0f) {}
};

class ChocolatePancake : public Pancake {
public:
	ChocolatePancake() : Pancake("Chocolate pancake", 260.0f) {}
};

class BananaPancake : public Pancake {
public:
	BananaPancake() : Pancake("Banana pancake", 260.0f) {}
};

class JamPancake : public Pancake {
public:
	JamPancake() : Pancake("Jam pancake", 190.0f) {}
};

class Icecream : public Meal {
private:
   std::string name;
   float price;
public:
   Icecream(const std::string &name, float price) : name(name), price(price) {}
   float getPrice() override{
       return price;
   }
   std::string getName() override{
       return name;
   }
   void info() override {
		std::cout << getName() << ", price: " << getPrice() << std::endl;
   }
};

class BananaSplit : public Icecream {
public:
	BananaSplit() : Icecream("Banana Split", 260) {}
};

class BerryScream : public Icecream {
public:
	BerryScream() : Icecream("Berry ice cream", 260) {}
};

class HotBrauni : public Icecream {
public:
	HotBrauni() : Icecream("Hot Brauni", 310) {}
};

class TuttiFruity : public Icecream {
public:
	TuttiFruity() : Icecream("Tutti Fruity", 240) {}
};

class Vanilla : public Icecream {
public:
	Vanilla() : Icecream("Vanilla ice cream", 180) {}
};

class Pistachio : public Icecream {
public:
	Pistachio() : Icecream("Pistachio ice cream", 230) {}
};

class SetMeal : public Meal {
protected:
	std::vector<Meal *> menu;
	std::string name;
	float price;
public:
	SetMeal(std::string nm) : name(nm), price(0) {}
	float getPrice() override {
		return price;
	}
	std::string getName() override {
		return name;
	}
	void info() override {
		std::cout << getName() << ", price: " << getPrice() << std::endl;
		for(auto it : menu) {
			std::cout << "      ";
			it->info();
		}
	}
	~SetMeal() {
		for(auto it : menu)
			delete(it);
	}
};

class Breakfast : public SetMeal {
public:
	Breakfast() : SetMeal("Breakfast (set meal)") { 
		menu.push_back(new PotatoPancake());
		menu.push_back(new BananaPancake());
		menu.push_back(new Vanilla());
		for(auto it : menu)
			price += it->getPrice();		
	}
};

class Lunch : public SetMeal {
public:
	Lunch() : SetMeal("Lunch (set meal)") { 
		menu.push_back(new MorningPancake());
		menu.push_back(new LunchPancake());
		menu.push_back(new ChocolatePancake());
		menu.push_back(new BananaSplit());
		for(auto it : menu)
			price += it->getPrice();		
	}	
};

class Dinner : public SetMeal {
public:
	Dinner() : SetMeal("Dinner (set meal)") { 
		menu.push_back(new MexicanPancake());
		menu.push_back(new MushroomPancake());
		menu.push_back(new FalafelPancake());
		menu.push_back(new HotBrauni());
		for(auto it : menu)
			price += it->getPrice();		
	}	
};

