...
public:
	...
	const int getHealthRating() const { return healthRating; }
	const float getCalories() const { return calories; }
	const float getProtein() const { return protein; }
	const float getCarbs() const { return carbs; }
	const float getFats() const { return fats; }
...
class Client : public Visitor{
	void visit(Ingredient * ing) override {
		std::cout 	<< "Name: " << ing->getName() 
					<< "\nHealthRating: " << ing->getHealthRating()
